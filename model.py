...
def cluster_18_simulation_model():
    ...
    chi_ambient = consts_d_nuc.CHI_AMBIENT
    
    # Load the SB locations
    SB_locations_b = torch.tensor(consts_d_nuc.GET_SB_LOCS)
        
    with pyro.plate("SBs", n_SBs):
        # Sample rho_SB, the scale factor of each bead
        rho_SB_loc_prior = consts_d_nuc.RHO_SB_LOC_PRIOR
        rho_SB_scale_prior = consts_d_nuc.RHO_SB_SCALE_PRIOR
        ...
        sigma_SB_loc_prior = consts_d_nuc.SIGMA_SB_LOC_PRIOR
        sigma_SB_scale_prior = consts_d_nuc.SIGMA_SB_SCALE_PRIOR
    ...
    d_drop_loc_prior = consts_d_nuc.D_DROP_LOC_PRIOR
    d_drop_scale_prior = consts_d_nuc.D_DROP_SCALE_PRIOR
    ...
    epsilon_capture_alpha_prior = consts_d_nuc.EPSILON_CAPTURE_ALPHA_PRIOR
    epsilon_capture_beta_prior = consts_d_nuc.EPSILON_CAPTURE_BETA_PRIOR
    ...
    d_nuc_loc_prior = consts_d_nuc.D_NUC_LOC_PRIOR
    d_nuc_scale_prior = consts_d_nuc.D_NUC_SCALE_PRIOR
    ...
    with pyro.plate("data", n_CBs):
        ...
        nuclei_x_n = pyro.sample('nuclei_x_n', dist.Uniform(consts_d_nuc.R_LOC_X - 1.5, consts_d_nuc.R_LOC_X + 1.5))
        nuclei_y_n = pyro.sample('nuclei_y_n', dist.Uniform(consts_d_nuc.R_LOC_Y - 1.5, consts_d_nuc.R_LOC_Y + 1.5))
...
def cluster_18_model(x: torch.Tensor, nuclei_x_n = None, nuclei_y_n = None):
    ...
    with pyro.plate("SBs", n_SBs):
        ...
        sigma_SB_loc_b = sigma_SB_loc_prior * torch.ones(n_SBs)
        sigma_SB_scale_b = sigma_SB_scale_prior * torch.ones(n_SBs)
        sigma_SB_b = pyro.sample("sigma_SB", dist.Cauchy(sigma_SB_loc_b, sigma_SB_scale_b))
        
        # # Sample sigma_SB, the diffusion radius of each bead
        # sigma_SB_loc_prior = consts_d_nuc.SIGMA_SB_LOG_NORMAL_LOC_PRIOR
        # sigma_SB_scale_prior = consts_d_nuc.SIGMA_SB_LOG_NORMAL_SCALE_PRIOR
        # sigma_SB_loc_b = sigma_SB_loc_prior * torch.ones(n_SBs)
        # sigma_SB_scale_b = sigma_SB_scale_prior * torch.ones(n_SBs)
